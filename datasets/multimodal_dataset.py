...
class MultimodalDataset(Dataset):
    def __init__(self, parser: CfgParser, phase: str, is_training: bool):
        ...
        self.ct_data: CTPEDataset3d = self._get_data(
            self.cfgs_ct, phase=phase, is_training=is_training)
        self.ehr_data: EHRDataset = self._get_data(self.cfgs_ehr, phase=phase)
        self.phase = phase

    def __getitem__(self, index):
        ct_volume, ct_dict = self.ct_data[index]
        ehr_feat, ehr_label = self.ehr_data[index]

        return ct_volume, ehr_feat, ct_dict['is_abnormal']

    def __len__(self):
        assert len(self.ct_data.window_to_series_idx) == len(
            self.ehr_data.ehr_data), "EHR data nums and CT data nums are not equal !"

        return len(self.ehr_data.ehr_data)
